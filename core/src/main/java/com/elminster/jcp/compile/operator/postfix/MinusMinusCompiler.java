 ...
 *
 * <p>Generates bytecode to decrement an integer variable while returning
 * the original (pre-decrement) value. Uses the efficient {@code IINC}
 * instruction for the decrement.
 *
 ...
 * IINC index, -1       // decrement variable by 1
 * }</pre>
 *
 * <h4>Example: y = x--</h4>
 * <pre>{@code
 * // If x is at index 1, y is at index 2
 * ILOAD 1              // load x's current value (before decrement)
 * IINC 1, -1           // decrement x by 1 (in-place)
 * ISTORE 2             // store old value in y
 * }</pre>
 *
 * <h4>Note:</h4>
 * <p>Post-decrement returns the value <i>before</i> the decrement, which is
 * why we load first, then decrement. The {@code IINC} instruction directly
